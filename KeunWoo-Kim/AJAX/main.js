...
function createElement(selectMovie) {
    return `
      <div class="cart-item">
        ...
        <b class="count">수량</b>
        <b class="amount" id="amount-${selectMovie.id}">1</b>
        <button button class="plumi" onclick="amountIncrease(${selectMovie.id})">
          <span>+</span>
        </button>
        <button button class="plumi" onclick="amountDecrease(${selectMovie.id})">
          <span>-</span>
        </button>
  ...
  function add(e, movies) {
    ...
    if (selectMovie && selectBlu) {
      movieCart.insertAdjacentHTML('beforeend', createElement(selectMovie));
      amounts[selectMovie.id] = 1;
    } else {
      alert("장바구니가 비어있습니다.");
    }
  }
  
  function amountDecrease(movieID) {
    if (amounts[movieID] > 1) {
      amounts[movieID]--;
      amountDetect(movieID).textContent = amounts[movieID];
    }
  }
  
  function amountIncrease(movieID) {
    amounts[movieID]++;
    amountDetect(movieID).textContent = amounts[movieID];
  }
  
  function amountDetect(movieID) {
    return document.querySelector(`#amount-${movieID}`);
  }
