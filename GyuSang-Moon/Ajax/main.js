...
function createElementDVD(selectMovie) {
  return `
          <div class="cart-item">
  ...
  function createElementBluray(selectMovie) {
      return `
              <div class="cart-item">
                  <h3 class="title">${selectMovie.title}</h3>
                  <p>장르: ${selectMovie.genre}</p>
...
function add(e, movies) {
  ...
  {
    movieCart.insertAdjacentHTML('beforeend', createElementBluray(selectMovie));
  }
  else if (selectMovie && selectDVD )
  {
    movieCart.insertAdjacentHTML('beforeend', createElementDVD(selectMovie));
  }
  else
  {
    alert("장바구니가 비어있습니다.");
